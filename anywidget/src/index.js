...
import { MystAnyModel } from "@myst-theme/anywidget";
import { MyST } from "myst-to-react";

export const MYST_PROP_CHILDREN = "myst#children";

export function AnyWidgetRenderer({ node }) {
  // basic validation
    ...
    import(esmModuleUrl)
      .then(async (mod) => {
        ...
        model.on(`change:${MYST_PROP_CHILDREN}`, () => {
          setChildren(model.get(MYST_PROP_CHILDREN));
        });
        ...
        if (useShadowDom) {
          const shadowRoot =
            rootEl.shadowRoot ?? rootEl.attachShadow({ mode: "open" });
      ...
      <details className="p-3 bg-gray-100 rounded border border-gray-300 cursor-pointer">
        ...
        <div className="pt-2 mt-2 space-y-1 text-xs border-t border-gray-200">
          <div className="text-gray-500">
            <span className="font-medium">Widget Module URL:</span>{" "}
            {esmModuleUrl}
          </div>
